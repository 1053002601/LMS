...
class LSGAVITBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads,
        ...
        # 引入分组点卷积和分组卷积
        self.gpwc = nn.Conv2d(dim, dim, groups=8, kernel_size=1)
        self.gc = nn.Conv2d(dim, dim, kernel_size=3, groups=8, padding=1, stride=1)
    ...
    def forward(self, x):
        ...
        x = self.relu(self.bn(self.gc(self.gpwc(x))))
...
class PatchEmbed(nn.Module):
    def __init__(self, img_size, patch_size, conv_embed_dim=4, in_chans=125, embed_dim=96, norm_layer=None):
        super().__init__()
        ...
        # 分组卷积运用到3D卷积操作
        groups_3d = 4  # 可根据需要调整分组数
        self.conv3d_features = nn.Sequential(
            nn.Conv3d(1, out_channels=conv_embed_dim, kernel_size=(3, 3, 3), padding=1, stride=1, groups=groups_3d),
        ...
        # 多尺度深度可分离卷积
        kernel_sizes = [3, 5, 7]
        self.multi_scale_depthwise_convs = nn.ModuleList([
            nn.Conv2d(in_channels=in_chans * conv_embed_dim, out_channels=in_chans * conv_embed_dim,
                      kernel_size=ks, padding=ks // 2, stride=1,
                      groups=in_chans * conv_embed_dim) for ks in kernel_sizes
        ])
        self.pointwise_conv_2d = nn.Conv2d(in_channels=in_chans * conv_embed_dim * len(kernel_sizes),
                                           out_channels=embed_dim, kernel_size=1)
        self.bn_2d = nn.BatchNorm2d(embed_dim)
    ...
    def forward(self, x):
        ...
        for depthwise_conv in self.multi_scale_depthwise_convs:
            feature = depthwise_conv(x)
            multi_scale_features.append(feature)
        x = torch.cat(multi_scale_features, dim=1)

        x = self.pointwise_conv_2d(x)
        x = self.relu_2d(self.bn_2d(x))
...
class LSGAVIT(nn.Module):
    def __init__(self, img_size, patch_size, in_chans=125, num_classes=1000,
...
model = LSGAVIT(
    img_size=2048,             # 输入图像尺寸
    patch_size=7,              # 增大patch_size以减少序列长度（原为3）
    in_chans=32,               # 输入通道数
    num_classes=5,             # 类别数
    embed_dim=128,             # 减小嵌入维度（原为192）
    depths=[2],                # 单层，两个transformer模块
    num_heads=[4],             # 4个注意力头，头维度=32
    mlp_ratio=2,               # 降低MLP比例
